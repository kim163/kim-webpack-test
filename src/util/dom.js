...
NodeList.prototype.classToggle = Element.prototype.classToggle = function(csName) {
  ...
  for(var i = 0; i < item.length; i++) {
    var allClass= item[i].className.split(" ") || [];
    var _csName=csName.split(" ") || [];
    for(var y=0;y<_csName.length;y++){
      var index = allClass.indexOf(_csName[y]);
      if(index>-1) {
        allClass.splice(index,1);
      } else {
        allClass.push(_csName[y]);
      }
      item[i].className =allClass.join(" ");
    }
  }
  return this;
}
